...
FREE_Start_ID :  //Limited match ID (mostly alpha)
   [#@$a-zA-Z_]* [a-zA-Z] [#@$a-zA-Z0-9_]* -> type(ID),mode(FREE);
...
FREE_NEWLINE :   NEWLINE { _modeStack.peek()!=FIXED_CalcSpec }? -> skip,pushMode(CheckComment),pushMode(First5); //Note: Removed popMode

FREE_SEMI : ';' {} -> popMode, pushMode(FREE_ENDED);  //Captures // immediately following the semi colon
